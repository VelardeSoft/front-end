    ...
    <div class="p-card p-shadow-2 mb-4">
      <div class="p-card-body">
        <div class="flex justify-content-between align-items-center mb-4">
          <div>
            <div class="p-card-title mb-0">{{ $t('rooms.create') }}</div>
            <div v-if="hotel" class="p-card-subtitle">{{ hotel.name }}</div>
          </div>
          <Button
...
const room = reactive({
  ...
  status: 'Available',
  description: '',
  image: 'https://placehold.co/600x400?text=Room+Image',
  amenities: []
});
...
const addAmenity = () => {
  if (newAmenity.value.trim()) {
    ...
    room.amenities.push(newAmenity.value.trim());
    newAmenity.value = '';
  }
};
